...
waitForStart(){
    ...
    do
        ...
        echo "Synapse server status: "${status}
        if [[ ${status} == "Online" ]]; then
            echo "Synapse server is online."
            exit 0
        fi
        if [[ "$status" != "Resuming" ]]; then
            echo "Synapse server is not resuming. Status: "${status}
            exit 1
        fi
        sleep 10
    done

    echo "Synapse server is not running after 20 minutes."
    exit 1
}
